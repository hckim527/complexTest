 ...
 * @param {Object} scopeApi 객체 컴포넌트 객체 또는 아이디(pageFrame Scope 경로를 포함한 Full Path Id)
...
scwin.getActiveWindowInfo = function ($p, scopeApi) {
  ...
  if (popupList.length > 0) {
    for (let i = popupList.length - 1; i > -1; i--) {
      if (document.activeElement.id + "_pageFrame" === popupList[i].$p.getFrameId()) {
  ...
  let findProgramList;
  if (popupWindow !== null) {
    // pageFrame Popup 방식으로 오픈된 팝업 화면
    activeInfo["type"] = "P";
...
scwin.openMenu = function ($p, menuNm, url, menuCode, paramObj, option) {
  ...
  if (url == "/") {
    ...
    if (layout == "T") {
      ...
      //start 동일메뉴 재오픈 체크
      let curParamData = $p.main().pfm_layout.scope.$p.getParameter("paramData") || {};
      let curMenuInfo = curParamData["menuInfo"];
      if (curMenuInfo) {
        let curMenuCd = curMenuInfo["menuCode"];
        if (curMenuCd == menuCode) {
          //이미 열려있는 메뉴면 리턴처리.
          return false;
        }
      }
...
scwin.openPopup = function ($p, url, opts, data) {
  return new Promise(function (resolve, reject) {
    ...
    let closeActionFunc = function () {
      ...
      if (!isClose) {
        reject(new Error("닫기 작업이 거부되었습니다."));
        return false;
      }
      ...
      if (!$c.util.isEmpty($p, popup) && !$c.util.isEmpty($p, popup.callbackParam)) {
        closeParam = $c.util.getJSON($p, popup.callbackParam);
        ...
        closeParam = $c.util.getJSON($p, popupWindow.$p.main().main._popup.callbackParam);
      } else {
      ...
      if (typeof callbackFunc === "function") {
        ret = callbackFunc(closeParam);
        if ($c.util.isEmpty($p, ret)) ret = true;
      }
      if (ret !== false) {
        resolve(closeParam);
      }
    ...
    scwin._openPopup($p, url, opts, data, closeActionFunc);
  });
};
 ...
 * @param {String} options.title [default : false]
 * @param {String} options.notMinSize [default : false]
 * @param {Object} data 팝업 화면에 전달할 데이터 객체
 * @param {Object} closeActionFunc 팝업이 닫힐 때 실행될 콜백 함수
 * @hidden Y
 * @exception 
 * @example
 */
scwin._openPopup = function ($p, url, opt, data, closeActionFunc) {
  data.menuInfo = {
    src: url
  };
  const _dataObj = {
  ...
  opt.type = opt.type || "pageFramePopup";
...
$c.win.messageBox($p, "alert", "보낼 메시지", "callback");

//confirm창을 띄울 경우
scwin.callback = function(){
    console.log("콜백 함수입니다.");
};
$c.win.messageBox($p, "confirm", "보낼 메시지", "callback");
 */
scwin.messageBox = function ($p, messageType, messageStr, closeCallbackFncName, opts) {
...
scwin.alert = function ($p, messageStr, closeCallbackFncName, opts) {
  return new Promise(function (resolve, reject) {
    ...
    $c.win.messageBox($p, "alert", messageStr, closeCallbackFncName, opts);
  });
};
...
scwin.confirm = function ($p, messageStr, closeCallbackFncName, opts) {
  return new Promise(function (resolve, reject) {
    ...
    $c.win.messageBox($p, "confirm", messageStr, closeCallbackFncName, opts);
  });
};
...
/**
 * @method 
 * @name setPageFrameSrc
 * @description 특정 pageFrame의 웹스퀘어 페이지(XML)을 변경한다.
 * @param {Object} pageFrameObj pageFrame 객체
 * @param {String} moveUrl 웹스퀘어 페이지(XML) 파일 경로
 * @param {Object} paramObj pageFrame을 set할 시 전달할 파라미터 객체
 * @hidden N
 * @exception 
 * @example
const param = {
...
$c.win.setPageFrameSrc(pfm_content, "/tmp/tmp01.xml", param);
 */
scwin.setPageFrameSrc = function ($p, pageFrameObj, moveUrl, paramObj) {
  paramObj = {
    "dataObject": {
      "type": "json",
      "name": "paramData",
  ...
  const contextPath = $c.sbm.getContextPath($p);
  return pageFrameObj.setSrc(contextPath + moveUrl, paramObj);
};

/**
 * @method 
 * @name getFrame
 * @description pageFrame안의 스크립트 영역에서 이 함수를 호출할 경우 자신을 감싼 pageFrame object를 반환한다. 전역스크립트에서 호출 시에는 null을 반환한다.
 * @returns {Object} 객체 자신을 감싼 pageFrame object
 * @hidden N
 * @exception 
 * @example
 ...
 * @method 
 * @name getParent
 * @description 자신의 부모 pageFrame 객체를 찾아 반환한다.
 * @returns {Object} 자신을 감싼 pageFrame object의 부모 pageFrame 객체
 * @hidden N
 * @exception 
 * @example
