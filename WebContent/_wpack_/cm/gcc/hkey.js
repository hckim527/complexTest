/*amd /cm/gcc/hkey.xml 23658 5bed6230ee09320ff13c5410da5373ede5c967fdb40adff14f0f785deb53ce00 */
 ...
 * Submssion(_sbm_selectShortcutList)의 실행은 config.xml의 pageFrame > postScript에 정의된 $c.win.processCommonData 함수에서 실행한다.
...
scwin.delEvent = function ($p, keyCode, options) {
  ...
  try {
    if (keyCode.lastIndexOf("+") > 0) {
      ...
      let lastKey = keyCode.slice(_idx + 1).toUpperCase();
    ...
    if (eventKey === "F1") {
      ...
      if (popupIndex > -1) {
        for (let i = popupIndex; popupIndex >= 0; popupIndex--) {
          if ($p.getAllPopupList()[i].options.frameMode === "pageFrame") {
